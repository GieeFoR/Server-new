...
new const String:PLUGIN_NAME[32] = "coditem_nabojeninji";
new const String:PLUGIN_AUTHOR[32] = "GieeF";
new const String:PLUGIN_VERSION[32] = "1.0";
new const String:PLUGIN_DESCRIPTION[64] = "Item COD - Naboje Ninji";
new const String:PLUGIN_URL[32] = "-";

new const String:item_name[] = "Naboje Ninji";
new const String:item_description[] = "Regenerujesz RNG procent zadanych obrażeń w postaci zdrowia";
...
new const item_minVal = 30;
new const item_maxVal = 70;
...
public OnAllPluginsLoaded() {
	CreateTimer(1.8, RegisterStart, 0);
}
...
public Action:OnPlayerTakeDamage(client, &attacker, &inflictor, &Float:damage, &damagetype) {
	...
	new healthRegeneration = RoundFloat(damage*player_itemValue[client]/100);
	new playerHealth = GetClientHealth(attacker);
	new playerMaxHealth = cod_getPlayerMaxHealth(attacker);
	SetEntData(attacker, FindDataMapInfo(attacker, "m_iHealth"), (playerHealth+healthRegeneration < playerMaxHealth)? playerHealth+healthRegeneration: playerMaxHealth);
	return Plugin_Continue;
}